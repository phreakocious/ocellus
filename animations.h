...
// Id layout (2026-07-16): eye/effect ids 0..37 are the stable per-owner range and never move.
...
// 45 = Swirl, and the next effect takes 46+ -- do NOT shift audio/debug; units in the field
// have those ids in saved configs. The debug ids (42..44) are interior holes in the playable
...
constexpr uint8_t QR_ID = EYE_COUNT + 21;         // id 34; per-owner QR code (bitmap from config.html; device only blits)
...
// ids 0..45; the debug trio (42..44) sits between the last two playables. Names are placeholders,
